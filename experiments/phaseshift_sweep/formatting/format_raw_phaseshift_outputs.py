#!/usr/bin/env python3
"""
format_raw_phaseshift_outputs.py

Convert phase-shift OCEAN output_signals.txt files into analysis-ready CSVs.
...
The formatter supports both the current flat layout:
  <phase_shift>/output_signals.txt

and the future Vtau-aware layout:
...
JOBS: dict[str, FormatJob] = {
    "1syn": FormatJob(
        kind="1syn",
        raw_subdir="phase_shift_1syn_ocean_output_v2",
        formatted_subdir="phase_shift_1syn",
        header=("time_s", "i_I56_Iout_A", "v_vpre_res_V", "v_vpre1_res_V"),
    ),
    "2syn": FormatJob(
        kind="2syn",
        raw_subdir="phase_shift_2syn_ocean_output_v2",
        formatted_subdir="phase_shift_2syn",
...
def discover_cases(raw_root_dir: Path, output_root_dir: Path) -> list[CaseInfo]:
    ...
    if not cases:
        raise FileNotFoundError(f"No {DEFAULT_INPUT_NAME} files found under {raw_root_dir}")
    return cases
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Format raw phase-shift OCEAN output_signals.txt files into CSVs."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.list_inputs:
        print("Phase-shift formatter inputs")
