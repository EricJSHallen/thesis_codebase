#!/usr/bin/env python3
"""
format_raw_phaseshift_vw_outputs.py

Convert Vw phase-shift OCEAN output_signals.txt files into analysis-ready CSVs.
...
Default formatted outputs:
  database/formatted/phase_shift_1syn_vw_v2/
  database/formatted/phase_shift_2syn_vw_v2/

The formatter only includes the Vw-aware layout:
  <vw_label>/<phase_shift>/output_signals.txt
"""
...
VW_DIR_RE = re.compile(r"^vw_\d+p\d+$")
...
JOBS: dict[str, FormatJob] = {
    "1syn": FormatJob(
        kind="1syn",
        raw_subdir="phase_shift_1syn_ocean_output_vw_v2",
        formatted_subdir="phase_shift_1syn_vw_v2",
        header=("time_s", "i_I56_Iout_A", "v_vpre_res_V", "v_vpre1_res_V"),
    ),
    "2syn": FormatJob(
        kind="2syn",
        raw_subdir="phase_shift_2syn_ocean_output_vw_v2",
        formatted_subdir="phase_shift_2syn_vw_v2",
...
def output_relative_path(raw_root_dir: Path, input_txt: Path) -> tuple[Path, str, str, str]:
    ...
    if len(parts) == 1:
        ...
        vw_label, phase_shift = parts
        case_name = f"{vw_label}/{phase_shift}"
        rel_csv = Path(vw_label) / f"{phase_shift}.csv"
    else:
        vw_label = parts[-2] if len(parts) >= 2 else ""
        phase_shift = parts[-1]
        case_name = "/".join(parts)
        rel_csv = Path(*parts[:-1]) / f"{phase_shift}.csv"
    return rel_csv, case_name, phase_shift, vw_label


def is_vw_case(raw_root_dir: Path, input_txt: Path) -> bool:
    rel_parent = input_txt.parent.relative_to(raw_root_dir)
    parts = rel_parent.parts
    return len(parts) == 2 and VW_DIR_RE.match(parts[0]) is not None
...
def discover_cases(raw_root_dir: Path, output_root_dir: Path) -> list[CaseInfo]:
    ...
    for input_txt in sorted(raw_root_dir.rglob(DEFAULT_INPUT_NAME)):
        if not is_vw_case(raw_root_dir, input_txt):
            continue

        rel_csv, case_name, phase_shift, vw_label = output_relative_path(raw_root_dir, input_txt)
        cases.append(
            CaseInfo(
                case_name=case_name,
    ...
    if not cases:
        raise FileNotFoundError(
            f"No nested vw_*p*/<phase_shift>/{DEFAULT_INPUT_NAME} files found under {raw_root_dir}"
        )
    return cases
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Format raw Vw phase-shift OCEAN output_signals.txt files into CSVs."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.list_inputs:
        print("Vw phase-shift formatter inputs")
