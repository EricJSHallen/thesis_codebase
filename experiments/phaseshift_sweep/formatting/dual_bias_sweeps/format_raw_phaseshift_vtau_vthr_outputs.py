#!/usr/bin/env python3
"""
format_raw_phaseshift_vtau_vthr_outputs.py

Convert Vtau/Vthr phase-shift OCEAN output_signals.txt files into analysis-ready CSVs.
...
Default formatted outputs:
  database/formatted/phase_shift_1syn_vtau_vthr_v2/
  database/formatted/phase_shift_2syn_vtau_vthr_v2/

The formatter only includes the dual-bias layout:
  <vtau_label>/<vthr_label>/<phase_shift>/output_signals.txt
"""
...
VTHR_DIR_RE = re.compile(r"^vthr_\d+p\d+$")
...
JOBS: dict[str, FormatJob] = {
    "1syn": FormatJob(
        kind="1syn",
        raw_subdir="phase_shift_1syn_ocean_output_vtau_vthr_v2",
        formatted_subdir="phase_shift_1syn_vtau_vthr_v2",
        header=("time_s", "i_I56_Iout_A", "v_vpre_res_V", "v_vpre1_res_V"),
    ),
    "2syn": FormatJob(
        kind="2syn",
        raw_subdir="phase_shift_2syn_ocean_output_vtau_vthr_v2",
        formatted_subdir="phase_shift_2syn_vtau_vthr_v2",
...
def output_relative_path(raw_root_dir: Path, input_txt: Path) -> tuple[Path, str, str, str, str]:
    rel_parent = input_txt.parent.relative_to(raw_root_dir)
    parts = rel_parent.parts
    if len(parts) != 3:
        raise ValueError(
            f"expected <vtau_label>/<vthr_label>/<phase_shift>, got {'/'.join(parts)!r}"
        )

    vtau_label, vthr_label, phase_shift = parts
    case_name = f"{vtau_label}/{vthr_label}/{phase_shift}"
    rel_csv = Path(vtau_label) / vthr_label / f"{phase_shift}.csv"
    return rel_csv, case_name, phase_shift, vtau_label, vthr_label


def is_dual_bias_case(raw_root_dir: Path, input_txt: Path) -> bool:
    rel_parent = input_txt.parent.relative_to(raw_root_dir)
    parts = rel_parent.parts
    return (
        len(parts) == 3
        and VTAU_DIR_RE.match(parts[0]) is not None
        and VTHR_DIR_RE.match(parts[1]) is not None
    )
...
def discover_cases(raw_root_dir: Path, output_root_dir: Path) -> list[CaseInfo]:
    ...
    for input_txt in sorted(raw_root_dir.rglob(DEFAULT_INPUT_NAME)):
        if not is_dual_bias_case(raw_root_dir, input_txt):
            continue

        rel_csv, case_name, phase_shift, vtau_label, vthr_label = output_relative_path(raw_root_dir, input_txt)
        cases.append(
            CaseInfo(
                case_name=case_name,
                phase_shift=phase_shift,
                vtau_label=vtau_label,
                vthr_label=vthr_label,
                input_txt=input_txt,
    ...
    if not cases:
        raise FileNotFoundError(
            f"No nested vtau_*p*/vthr_*p*/<phase_shift>/{DEFAULT_INPUT_NAME} files found under {raw_root_dir}"
        )
    return cases
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Format raw Vtau/Vthr phase-shift OCEAN output_signals.txt files into CSVs."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.list_inputs:
        print("Vtau/Vthr phase-shift formatter inputs")
