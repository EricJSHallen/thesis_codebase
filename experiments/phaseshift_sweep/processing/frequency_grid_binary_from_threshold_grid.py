...
threshold_grid_csv = Path(__file__).resolve().with_name("threshold_grid.csv")
output_csv = Path(__file__).resolve().with_name(
    "frequency_grid_binary_from_threshold_grid.csv"
)
...
def read_isi_values_from_threshold_grid() -> list[float]:
    if not threshold_grid_csv.exists():
        raise FileNotFoundError(f"Threshold grid CSV does not exist: {threshold_grid_csv}")

    isi_values: list[float] = []

    with threshold_grid_csv.open("r", newline="") as f:
        reader = csv.DictReader(f)

        if reader.fieldnames is None:
            raise ValueError(f"Threshold grid CSV is empty: {threshold_grid_csv}")
        if "isi_us" not in reader.fieldnames:
            raise ValueError("Threshold grid CSV is missing required column: isi_us")

        for row in reader:
            isi_us = row["isi_us"].strip()
            if not isi_us:
                continue
            isi_values.append(float(isi_us) * 1e-6)

    return format_isi(isi_values)


def validate_parameters(isi_values: list[float]) -> None:
    if total_time <= 0:
    ...
    if not isi_values:
        raise ValueError("isi_values must contain at least one value.")
    if any(isi_value <= 0 for isi_value in isi_values):
        raise ValueError("isi_values must contain only positive values.")
...
def write_binary_grid_csv(isi_values: list[float]) -> int:
...
def main() -> None:
    isi_values = read_isi_values_from_threshold_grid()
    validate_parameters(isi_values)
    rows_written = write_binary_grid_csv(isi_values)

    print(f"Threshold grid CSV: {threshold_grid_csv}")
    print(f"ISI values loaded: {len(isi_values)}")
    print(
