...
Interactive Matplotlib viewer for filled current-only Vthr phase-shift comparisons.

For each matching formatted 1syn/2syn CSV pair, this script displays two
stacked baseline-subtracted current plots with area filled to zero:
  1. 1syn i_I56_Iout_A
  2. 2syn i_I172_Iout_A + i_I56_Iout_A
...
def default_one_syn_dir(repo_root: Path) -> Path:
    path = repo_root / "database" / "formatted" / "phase_shift_1syn_vthr_v2"
    if not path.is_dir():
...
def default_two_syn_dir(repo_root: Path) -> Path:
    path = repo_root / "database" / "formatted" / "phase_shift_2syn_vthr_v2"
    if not path.is_dir():
...
def parse_vthr_sort_key(path: Path) -> tuple[int, float | str]:
    vthr_label = ""
    for part in path.parts:
        if part.startswith("vthr_"):
            vthr_label = part
            break
    if not vthr_label:
        return (0, "")

    value = vthr_label.removeprefix("vthr_").replace("p", ".")
    try:
...
def extract_vthr_label(path: Path) -> str | None:
    for part in path.parts:
        if part.startswith("vthr_"):
            return part
    return None
...
def normalize_vthr_label(value: str) -> str:
    return value if value.startswith("vthr_") else f"vthr_{value}"


def format_phase_shift_us(path: Path) -> str:
    return f"{parse_phase_shift_s(path) * 1e6:g}"


def format_voltage_label(label: str, prefix: str) -> str:
    return label.removeprefix(prefix).replace("p", ".")


def csv_sort_key(path: Path) -> tuple[tuple[int, float | str], float, str]:
    return (parse_vthr_sort_key(path), parse_phase_shift_s(path), str(path))
...
def plot_current_pair(rel_path: Path, one_syn_csv: Path, two_syn_csv: Path, index: int, total: int) -> None:
    ...
    one_i56 = subtract_initial(one_df[ONE_SYN_CURRENT]) * 1e9
    two_i172 = subtract_initial(two_df[TWO_SYN_I172])
    two_i56 = subtract_initial(two_df[TWO_SYN_I56])
    two_syn_sum = subtract_initial(two_df[TWO_SYN_I172] + two_df[TWO_SYN_I56]) * 1e9

    label = str(rel_path.with_suffix(""))
    vthr_label = extract_vthr_label(one_syn_csv) or "vthr_unknown"
    phase_shift_us = format_phase_shift_us(rel_path)
    voltage = format_voltage_label(vthr_label, "vthr_")
    title = rf"ISI = {phase_shift_us} $\mu s$, $V_{{thr}}$ = {voltage} V"
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--vthr", default=None, help="Only plot this exact Vthr label, e.g. vthr_0p7 or 0p7.")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        ...
        if args.vthr is not None:
            requested_vthr = normalize_vthr_label(args.vthr)
            pairs = [pair for pair in pairs if extract_vthr_label(pair[1]) == requested_vthr]
            if not pairs:
                raise FileNotFoundError(f"No CSV pairs found for vthr label: {requested_vthr}")
        ...
        if requested_vthr is not None:
            print(f"Vthr filter: {requested_vthr}")
