...
Compute scalar current integrals for matched formatted phase-shift CSV pairs.
...
def default_one_syn_dir(repo_root: Path) -> Path:
    path = repo_root / "database" / "formatted" / "phase_shift_1syn"
    if not path.is_dir():
...
def default_two_syn_dir(repo_root: Path) -> Path:
    path = repo_root / "database" / "formatted" / "phase_shift_2syn"
    if not path.is_dir():
...
def integrate_pair(rel_path: Path, one_syn_csv: Path, two_syn_csv: Path) -> IntegrationResult:
    phase_shift, vtau_label = parse_case_metadata(rel_path)
