...
Interactive Matplotlib viewer for filled current-only Vtau phase-shift comparisons.
...
def extract_vtau_label(path: Path) -> str | None:
    for part in path.parts:
        if part.startswith("vtau_"):
            return part
    return None
...
def normalize_vtau_label(value: str) -> str:
    return value if value.startswith("vtau_") else f"vtau_{value}"
...
def plot_current_pair(rel_path: Path, one_syn_csv: Path, two_syn_csv: Path, index: int, total: int) -> None:
    ...
    vtau_label = extract_vtau_label(one_syn_csv) or "vtau_unknown"
    phase_shift_us = format_phase_shift_us(rel_path)
    voltage = format_voltage_label(vtau_label, "vtau_")
    title = rf"ISI = {phase_shift_us} $\mu s$, $V_{{\tau}}$ = {voltage} V"
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--vtau", default=None, help="Only plot this exact Vtau label, e.g. vtau_0p7 or 0p7.")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        ...
        requested_vtau = None
        if args.vtau is not None:
            requested_vtau = normalize_vtau_label(args.vtau)
            pairs = [pair for pair in pairs if extract_vtau_label(pair[1]) == requested_vtau]
            if not pairs:
                raise FileNotFoundError(f"No CSV pairs found for vtau label: {requested_vtau}")
        ...
        if requested_vtau is not None:
            print(f"Vtau filter: {requested_vtau}")
