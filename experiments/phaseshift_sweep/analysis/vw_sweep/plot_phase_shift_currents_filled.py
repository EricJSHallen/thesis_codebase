...
Interactive Matplotlib viewer for filled current-only Vw phase-shift comparisons.
...
def extract_vw_label(path: Path) -> str | None:
    for part in path.parts:
        if part.startswith("vw_"):
            return part
    return None
...
def normalize_vw_label(value: str) -> str:
    return value if value.startswith("vw_") else f"vw_{value}"
...
def plot_current_pair(rel_path: Path, one_syn_csv: Path, two_syn_csv: Path, index: int, total: int) -> None:
    ...
    vw_label = extract_vw_label(one_syn_csv) or "vw_unknown"
    phase_shift_us = format_phase_shift_us(rel_path)
    voltage = format_voltage_label(vw_label, "vw_")
    title = rf"ISI = {phase_shift_us} $\mu s$, $V_{{w}}$ = {voltage} V"
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--vw", default=None, help="Only plot this exact Vw label, e.g. vw_0p7 or 0p7.")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        ...
        if args.vw is not None:
            requested_vw = normalize_vw_label(args.vw)
            pairs = [pair for pair in pairs if extract_vw_label(pair[1]) == requested_vw]
            if not pairs:
                raise FileNotFoundError(f"No CSV pairs found for vw label: {requested_vw}")
        ...
        if requested_vw is not None:
            print(f"Vw filter: {requested_vw}")
