...
Also place avg_integral_v6.py in the same directory.
...
def default_output_dir() -> Path:
    return Path(__file__).resolve().parent / "heatmap_nd_outputs_v8"
...
def make_colormap() -> LinearSegmentedColormap:
    ...
    cmap = LinearSegmentedColormap.from_list("heatmap_nd_v8_cmap", colors, N=512)
...
def compute_visualisation_dataframe(input_root: Path | None = None) -> tuple[pd.DataFrame, dict[str, object]]:
...
def make_output_stem(metadata: dict[str, object], output_dir: Path) -> Path:
    ...
    return output_dir / f"heatmap_nd_v8_{metadata['mode']}_{var}"
...
def generate_visualisation(
    ...
    df_values, metadata = compute_visualisation_dataframe(input_root=input_root)
