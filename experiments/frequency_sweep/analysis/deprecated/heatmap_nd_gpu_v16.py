...
def default_output_dir() -> Path:
    return Path(__file__).resolve().parent / "heatmap_nd_gpu_outputs_v16"
...
def make_colormap() -> LinearSegmentedColormap:
    ...
    cmap = LinearSegmentedColormap.from_list("heatmap_nd_gpu_v16_cmap", colors, N=512)
...
def create_3d_cube_heatmap(
    ...
    Performance controls:
        - alpha_threshold skips nearly invisible cubes.
...
def make_output_stem(metadata: dict[str, object], output_dir: Path) -> Path:
    ...
    return output_dir / f"heatmap_nd_gpu_v16_{metadata['mode']}_{var}"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Higher values are faster but hide more low-signal cubes. Default: 0.02."
        ),
    )
    parser.add_argument(
        "--stride",
