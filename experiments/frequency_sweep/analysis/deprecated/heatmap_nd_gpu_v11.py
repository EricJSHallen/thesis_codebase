...
from codebase.frequency_sweep.previews.deprecated.avg_integral_gpu_v9 import (
    TIME_COL,
    SUM_COL,
    MEAN_INTEGRAL_COL,
    STD_INTEGRAL_COL,
...
def default_output_dir() -> Path:
    return Path(__file__).resolve().parent / "heatmap_nd_gpu_outputs_v11"
...
def make_colormap() -> LinearSegmentedColormap:
    ...
    cmap = LinearSegmentedColormap.from_list("heatmap_nd_gpu_v11_cmap", colors, N=512)
...
def make_output_stem(metadata: dict[str, object], output_dir: Path) -> Path:
    ...
    return output_dir / f"heatmap_nd_gpu_v11_{metadata['mode']}_{var}"
