...
def default_output_dir() -> Path:
    return Path(__file__).resolve().parent / "heatmap_nd_gpu_outputs_v13"
...
def make_colormap() -> LinearSegmentedColormap:
    ...
    cmap = LinearSegmentedColormap.from_list("heatmap_nd_gpu_v13_cmap", colors, N=512)
...
def compute_exponential_alpha(
    ...
    So:
        t = 1   -> alpha = 1
        t = 0.5 -> alpha = exp(-k/2), small but nonzero
        t = 0   -> alpha = exp(-k), approximately 0 for moderate k
    """
...
def make_output_stem(metadata: dict[str, object], output_dir: Path) -> Path:
    ...
    return output_dir / f"heatmap_nd_gpu_v13_{metadata['mode']}_{var}"
