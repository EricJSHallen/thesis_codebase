...
def default_output_dir() -> Path:
    return Path(__file__).resolve().parent / "heatmap_nd_gpu_outputs_v15"
...
def make_colormap() -> LinearSegmentedColormap:
    ...
    cmap = LinearSegmentedColormap.from_list("heatmap_nd_gpu_v15_cmap", colors, N=512)
...
def create_3d_cube_heatmap(
    ...
    if alpha_threshold > 0:
        rendered_rows["_alpha_preview"] = rendered_rows[MEAN_INTEGRAL_COL].apply(
    ...
    if max_interactive_cubes is not None and len(rendered_rows) > max_interactive_cubes:
        rendered_rows = (
            rendered_rows
            .sort_values(MEAN_INTEGRAL_COL, ascending=False)
...
def make_output_stem(metadata: dict[str, object], output_dir: Path) -> Path:
    ...
    return output_dir / f"heatmap_nd_gpu_v15_{metadata['mode']}_{var}"
