...
# per-job Cadence project directories, and an automatic per-job ade_e.scs linker.
...
# If accidentally inside thesis_codebase/thesis_codebase, prefer the outer repo.
...
resolve_ade_e_source() {
  ...
  if [[ -z "$ADE_E_SOURCE" || ! -f "$ADE_E_SOURCE" ]]; then
    echo "WARNING: could not find ade_e.scs automatically." >&2
    echo "         Set ADE_E_SOURCE=/full/path/to/ade_e.scs before running this script." >&2
    ADE_E_SOURCE=""
  fi
}
...
; Supplied by simprep_isolated_netlist_v9.sh in the generated wrapper commands.
; CAD_PROJECT_DIR is job-specific and prevents shared-netlist collisions.
...
when(cadProjectDir
  ; projectDir(...) is not available in this OCEAN/SKILL environment.
...
ADE_E_SOURCE="$ADE_E_SOURCE"
NUM_JOBS="$NUM_JOBS"
JOB_DIR="$IPC_DIR/job\${job_index}"
PROJECT_DIR="\$CADENCE_PROJECT_DIR/job\${job_index}"

if [[ ! -d "\$JOB_DIR" ]]; then
  echo "ERROR: missing job directory: \$JOB_DIR" >&2
  exit 1
fi
if [[ ! -d "\$PROJECT_DIR" ]]; then
  echo "ERROR: missing Cadence project directory: \$PROJECT_DIR" >&2
  exit 1
fi
...
start_ade_linker() {
  if [[ -z "\$ADE_E_SOURCE" || ! -f "\$ADE_E_SOURCE" ]]; then
    echo "WARNING: ADE_E_SOURCE not available; ade_e.scs linker disabled: \$ADE_E_SOURCE" >&2
    return
  fi
  (
    while true
    do
      find "\$PROJECT_DIR" -name input.scs -type f 2>/dev/null | while read -r input
      do
        d=\$(dirname "\$input")
        [[ -e "\$d/ade_e.scs" ]] || ln -sf "\$ADE_E_SOURCE" "\$d/ade_e.scs"
      done
      find "\$PROJECT_DIR" -type d \( -name netlist -o -path "*/spectre/schematic/netlist" \) 2>/dev/null | while read -r d
      do
        [[ -e "\$d/ade_e.scs" ]] || ln -sf "\$ADE_E_SOURCE" "\$d/ade_e.scs"
      done
      sleep 1
  ...
  ade_linker_pid="\$!"
  echo "Started ade_e.scs linker pid=\$ade_linker_pid for \$PROJECT_DIR"
}

cleanup() {
  if [[ -n "\$ade_linker_pid" ]]; then
    kill "\$ade_linker_pid" 2>/dev/null || true
    wait "\$ade_linker_pid" 2>/dev/null || true
  fi
}
...
env \\
  CAD_NUM_JOBS="\$NUM_JOBS" \\
  CAD_JOB_INDEX="\$job_index" \\
  CAD_BATCH_EXIT=1 \\
  CAD_RUN_DIR="\$RUN_DIR" \\
  CAD_OUTPUT_DIR="\$OUTPUT_DIR" \\
  CAD_SPIKE_DIR="\$SPIKE_ROOT" \\
  CAD_PROJECT_DIR="\$PROJECT_DIR" \\
  CAD_ADE_E_SOURCE="\$ADE_E_SOURCE" \\
  ocean -nograph -restore "\$RUN_OCN"
EOF_WRAP
  chmod +x "$wrapper"
}

echo "=== Cadence/OCEAN isolated-netlist run setup v9 ==="
echo "Repo:                $REPO"
...
resolve_ade_e_source
if [[ -n "$ADE_E_SOURCE" ]]; then
  echo "ADE_E_SOURCE:        $ADE_E_SOURCE"
else
  echo "ADE_E_SOURCE:        not found; set ADE_E_SOURCE manually if SFE-868 occurs"
fi
echo
...
grep -nE "CAD_SPIKE_DIR|CAD_OUTPUT_DIR|CAD_PROJECT_DIR|asimenv.startup|projectDir|pwlBaseDir|st1BaseDir|st2BaseDir|outputBaseDir" "$RUN_OCN" | head -80 || true
echo
...
EOF_INFO

  {
    ...
    echo "; Each command runs the job wrapper, which starts an ade_e.scs linker before OCEAN."
    echo
    for i in $(seq 0 $((NUM_JOBS - 1))); do
      printf 'ipcBeginProcess("sh -c '\''%s/run_ocean_job.sh %d > %s/ocean_apply_job%d.log 2>&1'\''")\n\n' \
        "$RUN_DIR" "$i" "$LOG_DIR" "$i"
    done
  } > "$CIW_COMMANDS"
...
expected_for_job() {
  local j="\$1"
  if (( j >= TOTAL_CASES )); then echo 0; else echo \$(( (TOTAL_CASES - 1 - j) / NUM_JOBS + 1 )); fi
}
case "\${1:-summary}" in
  tail) tail -f "\$LOG_DIR"/ocean_apply_job*.log ;;
  condensed)
    tail -f "\$LOG_DIR"/ocean_apply_job*.log | grep -iE "spectre completes|Simulation completed successfully|Finished:|Finished assigned|ERROR|FATAL|SFE-|SPECTRE-|Cannot open|tran are not available|CAD_BATCH_EXIT|Key has expired|Total valid cases seen|Cases assigned|Cases skipped|syntax error|eval: not a function|undefined function|Using isolated Cadence projectDir|ade_e"
    ;;
  count) find "\$OUTPUT_DIR" -name output_signals.txt | wc -l ;;
  jobs)
    ...
    ;;
  errors)
    grep -H -E "ERROR|FATAL|SFE-|SPECTRE-|Cannot find|Cannot open|syntax error|lineread|eval: not a function|undefined function|Key has expired|ade_e" "\$LOG_DIR"/ocean_apply_job*.log 2>/dev/null | tail -100 || true
    ;;
  progress)
    watch -n 10 "n=\$(find '$OUTPUT_DIR' -name output_signals.txt | wc -l); total=$TOTAL_CASES; width=50; filled=\$((n*width/total)); empty=\$((width-filled)); bar=\$(printf '%0.s#' \$(seq 1 \$filled)); space=\$(printf '%0.s-' \$(seq 1 \$empty)); pct=\$(awk -v n=\$n -v t=\$total 'BEGIN {printf \"%.2f\", 100*n/t}'); printf '[%s%s] %s/%s complete (%s%%)\\n' \"\$bar\" \"\$space\" \"\$n\" \"\$total\" \"\$pct\"; echo; '$MONITOR' jobs; echo; echo -n 'ade_e failures: '; grep -H \"Cannot open the input file '\''ade_e.scs'\''\" '$LOG_DIR'/ocean_apply_job*.log 2>/dev/null | wc -l; echo; '$MONITOR' errors"
    ;;
  *)
...
echo "Wrapper:       $RUN_DIR/run_ocean_job.sh"
