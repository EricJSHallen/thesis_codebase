...
for helper in run_all_workers.sh run_spectre_worker.sh setup_spectre_env.sh select_cases.py monitoring_commands.sh; do
  ...
  case "$helper" in
    ...
    run_spectre_worker.sh) min_lines=45 ;;
    setup_spectre_env.sh) min_lines=35 ;;
    select_cases.py) min_lines=6 ;;
    monitoring_commands.sh) min_lines=6 ;;
  esac
...
echo "=== Plain Spectre sweep prep v6 ==="
echo "Repo:        $REPO_DIR"
