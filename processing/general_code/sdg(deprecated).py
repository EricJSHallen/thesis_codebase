...
output_root = Path("./spike_train_output")

num_spike_train_sets = 2      # Creates st_1, st_2, ..., st_n
max_frequency_hz = 20         # Creates 1_hz, 2_hz, ..., max_frequency_hz_hz
trials_per_frequency = 10     # Creates trial_1.csv, ..., trial_j.csv
...
def get_pulse_width() -> float:
    ...
    The generator uses this value for every spike in the output CSVs.
    """
    return pulse_width
...
def build_pulse_points(spike_starts: np.ndarray, duration_s: float) -> list[tuple[float, float]]:
    ...
    Each row in the output CSV represents a point in a PWL waveform:
...
def write_spike_train_csv(
    ...
    """
    Write one spike-train CSV.

    Metadata lines beginning with # are included at the top. Most data-analysis
    tools can skip these comment lines if needed.
    """
    if output_path.exists() and not overwrite_existing_files:
        raise FileExistsError(f"File already exists: {output_path}")

    realised_frequency_hz = spike_count / total_time

    with output_path.open("w", newline="") as f:
        f.write(f"# requested_mean_frequency_hz,{requested_frequency_hz}\n")
        f.write(f"# realised_mean_frequency_hz,{realised_frequency_hz:.12g}\n")
        f.write(f"# spike_count,{spike_count}\n")
        f.write(f"# total_time_s,{total_time:.12g}\n")
        f.write(f"# pulse_height_v,{pulse_height:.12g}\n")
        f.write(f"# pulse_width_s,{pulse_width:.12g}\n")

        writer = csv.writer(f)
        writer.writerow(["time_s", "voltage_v"])
        writer.writerows((f"{time:.12e}", f"{voltage:.12e}") for time, voltage in points)
    ...
    for st_index in range(1, num_spike_train_sets + 1):
        ...
        for frequency_hz in range(1, max_frequency_hz + 1):
            ...
            for trial_index in range(1, trials_per_frequency + 1):
                ...
                output_path = frequency_dir / f"trial_{trial_index}.csv"
