...
class ComputeBackend:
    ...
    def _cupy_usable() -> bool:
        """
        Return True only if CuPy can do a real GPU operation.

        Merely detecting a CUDA device is not sufficient. On some systems CuPy
        can see the GPU but fails when it compiles/loads its reduction kernels.
        This warm-up catches that failure before the main loop starts.
        """
        if cp is None:
            return False
        ...
        try:
            ...
            test = cp.asarray([1.0, 2.0, 3.0], dtype=cp.float64)
            result = cp.sum(cp.maximum(test - 1.0, 0.0))
            float(cp.asnumpy(result))

            # Synchronise so delayed CUDA errors are raised here.
            cp.cuda.Stream.null.synchronize()
            return True

        except Exception as exc:
            print(
                "CuPy/CUDA was detected but failed a warm-up GPU operation. "
                "Falling back if --backend auto was used. "
                f"CuPy error: {type(exc).__name__}: {exc}"
            )
            return False
    ...
    def integrate_excess_from_aligned_arrays(self, time_s: np.ndarray, aligned_voltages: list[np.ndarray]) -> float:
        ...
        if self.name == "cupy":
            try:
                ...
                # CuPy has renamed trapz to trapezoid in newer NumPy-compatible
                # APIs, so support both names.
                if hasattr(cp, "trapezoid"):
                    gpu_integral = cp.trapezoid(gpu_excess, gpu_time)
                else:
                    gpu_integral = cp.trapz(gpu_excess, gpu_time)

                value = float(cp.asnumpy(gpu_integral))
                cp.cuda.Stream.null.synchronize()
                return value

            except Exception as exc:
                raise RuntimeError(
                    "CuPy/CUDA failed during the GPU integration kernel. "
                    "Run with --backend numba or --backend cpu, or fix the CUDA/CuPy installation. "
                    f"Original CuPy error: {type(exc).__name__}: {exc}"
                ) from exc
...
def default_output_csv() -> Path:
    return Path(__file__).resolve().parent / "average_integrated_voltage_difference_gpu_v9.csv"
