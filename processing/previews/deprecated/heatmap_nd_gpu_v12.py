...
def default_output_dir() -> Path:
    return Path(__file__).resolve().parent / "heatmap_nd_gpu_outputs_v12"
...
def make_colormap() -> LinearSegmentedColormap:
    ...
    cmap = LinearSegmentedColormap.from_list("heatmap_nd_gpu_v12_cmap", colors, N=512)
...
def create_3d_cube_heatmap(
    ...
    Visual encoding:
        ...
        - opacity (alpha) is normalized so that:
            lowest scalar -> alpha 0
            highest scalar -> alpha 1
    """
...
def make_output_stem(metadata: dict[str, object], output_dir: Path) -> Path:
    ...
    return output_dir / f"heatmap_nd_gpu_v12_{metadata['mode']}_{var}"
