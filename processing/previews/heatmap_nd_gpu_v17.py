...
NORMALIZED_MEAN_COL = "mean_voltage_difference_integral_per_second_v"
NORMALIZED_STD_COL = "std_voltage_difference_integral_per_second_v"
NORMALIZED_MIN_COL = "min_voltage_difference_integral_per_second_v"
NORMALIZED_MAX_COL = "max_voltage_difference_integral_per_second_v"
MEAN_SIM_DURATION_COL = "mean_simulation_duration_s"
...
def default_output_dir() -> Path:
    return Path(__file__).resolve().parent / "heatmap_nd_gpu_outputs_v17"
...
def make_colormap() -> LinearSegmentedColormap:
    ...
    cmap = LinearSegmentedColormap.from_list("heatmap_nd_gpu_v17_cmap", colors, N=512)
...
def compute_integral_and_duration_from_csv_paths(
    csv_paths: Sequence[Path],
    backend: ComputeBackend,
) -> tuple[float, float]:
    """
    Compute the integrated excess-voltage scalar and infer simulation duration
    from the aligned waveform time grid.

    duration_s = max(time_s) - min(time_s)

    For the current generated data this should be 0.5 s, but this value is not
    hard-coded.
    """
    ...
    if common_time.size < 2:
        raise ValueError("Could not infer duration from fewer than two time points.")

    duration_s = float(common_time[-1] - common_time[0])
    if duration_s <= 0:
        raise ValueError("Could not infer a positive simulation duration.")

    integral_vs = backend.integrate_excess_from_aligned_arrays(common_time, aligned_voltages)
...
def compute_average_scalar_for_frequency_setting(
    ...
    trial_normalized_values: list[float] = []
    ...
    for case in iter_sweep_cases_for_frequency_setting(
        ...
        integral_vs, duration_s = compute_integral_and_duration_from_csv_paths(
            case.csv_paths,
            backend=backend,
        )
        trial_integrals.append(integral_vs)
        trial_normalized_values.append(integral_vs / duration_s)
    ...
    normalized_values = np.asarray(trial_normalized_values, dtype=float)
    durations = np.asarray(trial_durations, dtype=float)
    ...
    row: dict[str, object] = {
        ...
        NORMALIZED_MEAN_COL: float(np.mean(normalized_values)),
        NORMALIZED_STD_COL: float(np.std(normalized_values, ddof=0)),
        NORMALIZED_MIN_COL: float(np.min(normalized_values)),
        NORMALIZED_MAX_COL: float(np.max(normalized_values)),
        MEAN_SIM_DURATION_COL: float(np.mean(durations)),
...
def create_2d_heatmap(
    ...
    ax.set_title("2D heatmap of duration-normalized averaged voltage difference")
    ax.set_xticks(nice_x_ticks)
    ax.set_yticks(nice_y_ticks)

    cbar = fig.colorbar(mesh, ax=ax, pad=0.02)
    cbar.set_label("Mean excess voltage after duration normalization / V")
...
def compute_exponential_alpha(
    ...
    """
    Exponential opacity mapping with zero floor.

    Design goal:
        - highest values -> opacity 1
        - middle values -> small but nonzero opacity
        - low values -> approximately 0
...
def create_3d_cube_heatmap(
    ...
    Visual encoding:
        ...
        - opacity (alpha) follows an exponential mapping with zero floor:
            highest scalar values are opaque
            middle values are close to 0 but still visible
            low values are approximately 0

    Performance controls:
        - alpha_threshold skips cubes whose opacity is near zero.
        - stride keeps only every Nth cube along each axis.
        - max_interactive_cubes keeps only the strongest cubes if the rendered
          cube count is still too large.
    """
    ...
    c = df_values[NORMALIZED_MEAN_COL].to_numpy(dtype=float)
    ...
    if alpha_threshold > 0:
        rendered_rows["_alpha_preview"] = rendered_rows[NORMALIZED_MEAN_COL].apply(
    ...
    if max_interactive_cubes is not None and len(rendered_rows) > max_interactive_cubes:
        rendered_rows = (
            rendered_rows
            .sort_values(NORMALIZED_MEAN_COL, ascending=False)
    ...
    for _, row in rendered_rows.iterrows():
        ...
        val = float(row[NORMALIZED_MEAN_COL])
    ...
    title = "3D cube heatmap of duration-normalized averaged voltage difference"
    ...
    # Leave room below the 3D plot for interactive rotation sliders.
...
def make_output_stem(metadata: dict[str, object], output_dir: Path) -> Path:
    ...
    return output_dir / f"heatmap_nd_gpu_v17_{metadata['mode']}_{var}"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0.01,
        help=(
            "3D mode only. Skip cubes with opacity below this value. "
            "Higher values are faster but hide more low-signal cubes. Default: 0.01."
        ),
    )
    parser.add_argument(
        "--stride",
        ...
        default=1,
        help=(
            "3D mode only. Render only every Nth cube along each axis. "
            "Use 2 or 3 for faster interactive rotation. Default: 1."
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "3D mode only. If set, render only the strongest N cubes after "
            "thresholding/striding. Useful for smooth interactive rotation."
        ),
    )
